function Lugia(x, y, track) {
  ...
  this.x = x;
  this.y = y;
  this.$sprite = $('<div id="standing_lugia"><img src="public/img/lugia_standing.gif"/></div>');
...
Lugia.prototype.run = function() {
  var old_x = this.x;
  $('#standing_lugia img').attr('src', 'public/img/lugia_flying.gif')
  this.x += 100;
  if  (((this.x - this.width / 2) < 0) ||
      ((this.x + this.width / 2) > this.track.width)) {
      this.x = old_x;
      this.collided = true;
      $('#standing_lugia img').attr('src', 'public/img/lugia_standing.gif')
  }
  this.updateSpritePosition();
}
